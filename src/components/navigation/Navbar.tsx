...
export const Navbar: React.FC<Props> = ({ logo, logo_vivero }) => {
  ...
  return (
    <nav className="bg-white p-4 flex flex-wrap items-center justify-between">
      ...
      <div
        ref={menuRef}
        className={`w-full md:block md:w-auto ${isMobileMenuOpen ? "fixed top-0 left-0 bg-gray-200 p-4 z-9" : "hidden"}`}
      >
        <ul className="navbar-bd flex flex-col items-center md:flex-row md:items-center md:justify-end text-base text-grey-800">
          <li className="md:ml-6 mt-3 md:mt-0 w-full md:w-auto">
            <NavLink to={"/"} className="hover:text-grey-800 block text-center">
              Inicio
            </NavLink>
          </li>
          <li className="md:ml-6 mt-3 md:mt-0 w-full md:w-auto">
            <NavLink
              to={"/about"}
              className="hover:text-grey-950 block text-center"
            >
              Acerca de
            </NavLink>
          </li>
          {user ? (
            <li className="md:ml-6 mt-3 md:mt-0 w-full md:w-auto">
              <NavLink
                to={"/reports"}
                className="hover:text-grey-800 block text-center"
              >
                Reportes
              </NavLink>
            </li>
      ...
      {user ? (
        <Logout visible={openModal} action={handlerModal} />
      ) : (
        <Login visible={openModal} action={handlerModal} />
      )}
    </nav>
  );
};
